...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a German learning lesson (text + vocab + phrases + quiz + voice)."
    )
    parser.add_argument(
        "--level",
        required=True,
        choices=sorted(LEVEL_WORD_TARGETS.keys()),
        help="CEFR level",
    )
    parser.add_argument(
        "--topic",
        default=None,
        help="Free-form topic. Required when --source is omitted; optional steering hint when --source is a URL.",
    )
    parser.add_argument(
        "--source",
        default=None,
        help="Optional source URL. If provided, the LLM rewrites that page's main content at the target level.",
    )
    parser.add_argument(
        "--voice-id",
        default=None,
...
def main() -> None:
    ...
    if not args.source and not args.topic:
        sys.exit("error: --topic is required when --source is not provided")
    ...
    lesson = generate_lesson(level=args.level, topic=args.topic, source_url=args.source)

    voice_tmp_path = LESSONS_DIR / f".tmp-{timestamp}-latest.mp3"
    ...
    lesson_record = {
        ...
        "topic": args.topic,
        "source": (
            {"type": "url", "url": args.source} if args.source else {"type": "ai"}
        ),
    ...
    if saved_voice_path:
        voice_tmp_path.rename(LATEST_AUDIO_PATH)
